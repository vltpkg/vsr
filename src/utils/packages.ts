...
export function extractPublisher(parsed: Record<string, unknown>) {
  const npmUser = parsed._npmUser
  if (npmUser && typeof npmUser === 'object') {
    const user = npmUser as Record<string, unknown>
    const username =
    ...
    const name =
      typeof user.name === 'string' ? user.name : username
    return {
      username,
      email,
      name,
...
export function slimManifest(
  ...
  try {
    ...
    if (parsed.deprecated !== undefined)
      slimmed.deprecated = parsed.deprecated
    const publisher = extractPublisher(parsed as Record<string, unknown>)
    if (publisher) slimmed.publisher = publisher
